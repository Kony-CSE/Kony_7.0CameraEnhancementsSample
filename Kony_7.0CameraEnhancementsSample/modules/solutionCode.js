function setDefaultPhotoOptions(){
  ...
  frmPhoto.camera.cameraOptions={flashMode: constants.FLASH_MODE_AUTO};
}
...
function setDefaultVideoOptions(){
  ...
  frmVideo.camera.cameraOptions={flashMode: constants.FLASH_MODE_AUTO};
  frmVideo.camera.videoQualityLevel=frmVideo.listboxVideoQuality.masterData[0][0]
  //#ifdef iphone
  	frmVideo.camera.videoFormat=constants.CAMERA_VIDEO_FORMAT_MP4;
  //#endif
}


function setCameraSource(){
  if (currentForm.listboxCameraSource.selectedKeyValue[1] == "Default")
  	currentForm.camera.cameraSource=constants.CAMERA_SOURCE_DEFAULT;
  else if (currentForm.listboxCameraSource.selectedKeyValue[1] == "Front")
     	currentForm.camera.cameraSource=constants.CAMERA_SOURCE_FRONT;
   else
     	currentForm.camera.cameraSource=constants.CAMERA_SOURCE_REAR;
}
...
function setCameraFocusMode(){
  //#ifdef android
  	if (currentForm.id == "frmVideo")
     	currentForm.camera.focusMode =constants.CAMERA_FOCUS_MODE_CONTINUOUS;
   else
     	currentForm.camera.focusMode =constants.CAMERA_FOCUS_MODE_AUTO;
    //#endif
}
...
function setCameraFlashMode(){
  ...
  /* if (currentForm.id == "frmVideo"){
        ...
        if (currentForm.listboxFlashMode.selectedKeyValue[1] == "Auto")
            currentForm.camera.cameraOptions={flashMode: constants.FLASH_MODE_AUTO};
        else if(currentForm.listboxFlashMode.selectedKeyValue[1] == "On")
          	currentForm.camera.cameraOptions={flashMode:constants.FLASH_MODE_ON}; 
   		else if(currentForm.listboxFlashMode.selectedKeyValue[1] == "Off")
         	 currentForm.camera.cameraOptions={flashMode:constants.FLASH_MODE_OFF}; 
   		else
         	 currentForm.camera.cameraOptions={flashMode:constants.FLASH_MODE_ALWAYS_ON}; 
  //}
...
function setVideoType(){
  //#ifdef iphone
   if (currentForm.listboxVideoType.selectedKeyValue[1] == "MP4")
     	currentForm.camera.videoFormat=constants.CAMERA_VIDEO_FORMAT_MP4;
   else
     	currentForm.camera.videoFormat=constants.CAMERA_VIDEO_FORMAT_MOV;
    //#endif
}
